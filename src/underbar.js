(function() {
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(item){
      modifiedArray.push(iterator(item));
    });
  ...
  _.every = function(collection, iterator) {
    ...
    return _.reduce(collection, function(allMatched, item){
      if(iterator){
        return Boolean(allMatched && iterator(item));
      } else {
        return Boolean(allMatched && item);
      }
    }, true);
  };
  ...
  _.delay = function(func, wait) {
    // Turn arguments array into an array with map
    var args = _.map(arguments, function(item){
      return item;
    });
    // Take off the first two arguments that contain the func and wait
    args.splice(0,2);
  ...
  _.zip = function() {
    var args = arguments;
    var zipped = [];
    zipped.length = _.reduce(arguments, function(maxLength, item){
    ...
    _.each(zipped, function(item, i){
      zipped[i] = _.reduce(args, function(row, item){
        return row.concat(item[i]);
      },[])
    })
    return zipped;
  };
  ...
  _.flatten = function(nestedArray, result) {
    if(Array.isArray(nestedArray)){
      return _.reduce(nestedArray, function(flatArray, item){
        return flatArray.concat(_.flatten(item));
      },[]);
    } else {
      return nestedArray;
    }
